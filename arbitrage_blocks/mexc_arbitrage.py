# Алгоритми для пошуку арбітражу SPOT-Ф'ЮЧЕРСИ та МАРЖА-Ф'ЮЧЕРСИ на MEXC (v4.0m_08-08-25)
import asyncio

async def get_spot_futures_arbitrage(mexc, symbols, config):
    results = []
    spot_symbols_set = set([s["symbol"] for s in symbols])
    futures_symbols_set = set(mexc.get_futures_symbols())
    ...
    for symbol_data in filtered_symbols:
        ...
        spot_price = mexc.get_price(symbol, category="spot")
        futures_price = mexc.get_price(symbol, category="linear")
...
async def get_margin_futures_arbitrage(mexc, symbols, config):
    results = []
    spot_symbols_set = set([s["symbol"] for s in symbols])
    futures_symbols_set = set(mexc.get_futures_symbols())
    common_symbols = spot_symbols_set & futures_symbols_set
    ...
    for symbol_data in filtered_symbols:
        symbol = symbol_data["symbol"]
        margin_price = mexc.get_price(symbol, category="margin")
        futures_price = mexc.get_price(symbol, category="linear")
